		...
		<table>		
			<?php
				...
				foreach($affichage_personne as $personne){
					...
					$id_valeur=$personne['id'];		
					if (!$test_amitie){
						echo "<form method='POST'>";
						echo "<td> <button type='submit' name='tralala' value = '".$key."' formaction='router.php?handler=Session&action_du_plouc=ajouter_un_pote'>Ajouter un ami!</button> </td>";	
						...
						echo "<form method='POST'>";
						echo "<td> <button type='submit' name='tralala' value = '".$key."' formaction='router.php?handler=Session&action_du_plouc=messenger'>Causer ac mon pote!</button> </td>";	
